...
import { Shield, CheckCircle, BarChart3, Eye, Trash2, MapPin, FileText, Globe } from 'lucide-react'
...
export function CookieConsentBanner({ className }: CookieConsentBannerProps) {
    ...
    useEffect(() => {
        if (gpcChecked && gpcEnabled && consent?.analytics !== false) {
            // If GPC is enabled and analytics preference is not explicitly false, opt-out.
            // This ensures GPC signal overrides previous settings if user hasn't opted out.
            updatePreferences({ analytics: false });
            // Optionally close banner after GPC auto-opt-out, but keep it open for user awareness.
            // closeBanner(); 
        }
    }, [gpcChecked, gpcEnabled, consent?.analytics, updatePreferences]); // Add dependencies

    const onAccept = useCallback(() => {
        acceptAll();
        // If GPC is enabled, ensure analytics remain false even if user clicks "Accept All"
        // This is a strict interpretation of GPC overriding explicit consent for "sale/share"
        if (gpcEnabled) {
             updatePreferences({ analytics: false });
        }
    }, [acceptAll, gpcEnabled, updatePreferences])

    const onReject = useCallback(() => {
        rejectNonEssential();
         // If GPC is enabled, ensure analytics remain false even if user clicks "Reject Non-Essential"
        if (gpcEnabled) {
             updatePreferences({ analytics: false });
        }
    }, [rejectNonEssential, gpcEnabled, updatePreferences])

    const onToggleAnalytics = useCallback(() => {
        updatePreferences({ analytics: !consent?.analytics }) 
    ...
    const banner = (
        <>
            ...
            <div
                className="cookie-consent-container fixed inset-x-0 bottom-0 sm:bottom-2 z-[2147483648] px-2 sm:px-4 safe-area-inset-bottom"
                style={{
            ...
            >
                <div
                    ...
                    className={cn(
                        'w-full max-w-none mx-auto bg-white dark:bg-slate-950 border border-border rounded-2xl shadow-lg',
                        'pointer-events-auto',
                        'overflow-hidden',
                        className
                    )}
                        ...
                        <div className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-2">
                            ...
                            >
                                ...
                                <div className="min-w-0">
                                    <h3 className="font-semibold text-foreground text-sm">Privacy Choices</h3>
                                    <p className="text-[11px] sm:text-xs text-muted-foreground leading-tight">
                                        Essential cookies only. Analytics require your consent.
                                    </p>
                                </div>
                            </div>

                            <div className="flex items-center gap-1 flex-shrink-0 justify-end min-w-0">
                                <div className="flex gap-1 items-center">
                                    ...
                                    <Button
                                        ...
                                        variant="outline"
                                        size="sm"
                                        className="text-red-500 border-red-500/60 hover:bg-red-50 px-2 py-1"
                                    >
                                        Reject All
                                    </Button>
                                </div>
                            ...
                            <div className="mt-2 pt-2 border-t border-border">
                                <div className="space-y-3 text-sm max-h-[300px] sm:max-h-none overflow-y-auto">
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <div className="p-1.5 bg-blue-500/10 border border-blue-500/20 rounded-lg text-[11px] leading-tight">
                                                ...
                                                <p className="text-xs text-muted-foreground">
                                                    Which pages are visited (anonymous)
                                                </p>
                                            </div>
                                            ...
                                            <div className="p-1.5 bg-blue-500/10 border border-blue-500/20 rounded-lg text-[11px] leading-tight">
                                                ...
                                                <p className="text-xs text-muted-foreground">
                                                    Current: <span className="text-blue-500 font-semibold">{deviceType}</span> (anonymous)
                                                </p>
                                            </div>
                                            ...
                                            <div className="p-1.5 bg-green-500/10 border border-green-500/20 rounded-lg text-[11px] leading-tight">
                                                ...
                                                <p className="text-xs text-muted-foreground">
                                                    Keep you signed in, manage preferences (required)
                                                </p>
                                            </div>
                                    ...
                                    <div className="flex items-center justify-between p-2 bg-muted/80 rounded-lg">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-muted-foreground">
                                                    Help us improve the site. This data is **not** used for cross-context behavioral advertising.
                                                </p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <button
                                                ...
                                                role="switch"
                                                aria-checked={consent?.analytics === true ? 'true' : 'false'}
                                                className={cn(
                                            ...
                                            >
                                                <span
                                                    className={cn(
                                                        ...
                                                        consent?.analytics === true ? 'translate-x-6' : 'translate-x-1'
                                                    )}
                                                />
                                            </button>
                                            <span className="text-xs text-muted-foreground">
                                                {consent?.analytics === true ? 'On' : 'Off'}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="text-[11px] text-muted-foreground flex flex-col gap-2">
                                        <p>
                                            <strong>Privacy Promise:</strong> We only collect anonymous usage data to improve our service if you explicitly consent.
                                            No personal information is collected, no tracking across other sites, and we never sell or share your data to third parties for cross-context behavioral advertising or similar purposes.
                                        </p>
                                        ...
                                        {/* New: Do Not Sell or Share Link */}
                                        <Link
                                            href="/do-not-sell"
                                            className="group block p-2 bg-yellow-500/10 border border-yellow-500/20 rounded-lg transition-all duration-200"
                                        >
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-2">
                                                    <Shield className="h-4 w-4 text-yellow-500" />
                                                    <span className="font-medium text-foreground">Do Not Sell or Share My Personal Information</span>
                                                </div>
                                                <div className="text-yellow-500 group-hover:translate-x-1 transition-transform">→</div>
                                            </div>
                                            <p className="text-[10px] mt-1 text-muted-foreground leading-tight">
                                                Manage your privacy rights under CCPA/CPRA.
                                            </p>
                                        </Link>
