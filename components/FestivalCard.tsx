...
export default function FestivalCard({ festival, surface = 'paper' }: FestivalCardProps) {
  ...
  const laurelScaleClass = festival.name === 'Tees Valley International Film Festival' ? 'scale-[2.25]' : festival.name === 'Coronado Island Film Festival' ? 'scale-[1.65] translate-y-2' : festival.name === 'Sogni Elettrici / Electricdreams International Film Festival' ? 'scale-[1.25]' : '';
  ...
  const laurel = surface === 'navy' ? (
    ...
    <>
      <Image src={festival.laurelDark} alt={`${festival.name} ${festival.recognition} laurel`} width={720} height={320} className={`${imageClass} ${laurelScaleClass} dark:hidden ${festival.name === 'Brooklyn SciFi Film Festival' || festival.name === 'GenreBlast Film Festival' || festival.name === 'Coronado Island Film Festival' || festival.name === 'Sogni Elettrici / Electricdreams International Film Festival' ? 'brightness-0' : ''} ${festival.name === 'GenreBlast Film Festival' ? 'translate-y-px scale-110' : ''}`} />
